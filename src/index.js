...
class QuantaSDK extends CoreBridge {
  constructor (config, options = { global: true }) {
    super()
  ...
  async checkAuthentication (clientId, token) {
    try {
      const result = await this.fetch(`${ENDPOINTS.checkAuthentication}?access_token=${token}&client_id=${clientId}`)
  ...
  async initial (config, options) {
    ...
    if (this.user && this.user.token && this.user.clientId) {
      const user = await this.checkAuthentication(this.user.clientId, this.user.token)
      if (user) {
